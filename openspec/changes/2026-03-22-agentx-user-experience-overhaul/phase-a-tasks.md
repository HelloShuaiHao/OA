...
- [x] 流程选择支持规则配置（不需要手动输入 workflow_id）
- [x] 后台自动生成和维护技术映射
- [x] 数字员工在组织架构中可见
...
**验收标准：**
- [x] 路由配置正确，菜单显示正常
- [x] 列表页面可以访问，URL: `/agentx/agents`
- [x] 详情页面可以访问，URL: `/agentx/agents/:id`
- [x] 页面布局符合设计稿（Figma/蓝湖链接）
- [x] 响应式布局，支持 1920x1080 和 1366x768 分辨率
- [x] 无 console 错误
- [x] 通过 ESLint 检查
...
**验收标准：**
- [x] 列表可以正常加载数据
- [x] 搜索功能正常，输入名称后点击搜索，列表更新
- [x] 筛选功能正常，选择部门/状态后列表更新
- [x] 分页功能正常，切换页码后列表更新
- [x] 启用/停用功能正常，点击后状态更新，卡片样式变化
- [x] 删除功能正常，弹出确认框，确认后删除成功
- [x] 空状态显示正常（无数据时显示空状态图）
- [x] 加载状态显示正常（loading 动画）
- [x] 错误状态显示正常（API 失败时显示错误提示）
...
- [x] 表单字段显示正常
- [x] 部门下拉框可以加载部门树
- [x] 头像选择器可以选择预设图标（至少 10 个）
- [x] 头像上传功能正常，支持 jpg/png，最大 2MB
- [x] 表单验证正常，必填项为空时显示错误提示
- [x] 点击"下一步"，验证通过后进入步骤 2
- [x] 点击"取消"，弹出确认框，确认后返回列表
...
- [x] 选中模板后，显示模板详细说明
- [x] 选择"自定义"，不自动填充能力和流程
- [x] 点击"上一步"，返回步骤 1，数据保留
- [x] 点击"下一步"，进入步骤 3，自动填充能力
...
**验收标准：**
- [x] 可以加载 Flowable 流程定义列表
- [x] 流程列表显示正常（名称、版本、分类）
- [x] 可以勾选多个流程
- [x] 点击"查看流程图"，弹出流程图预览
- [x] 选择策略切换正常
- [x] 选择"规则选择"，显示规则配置器
- [x] 选择"AI 自动选择"，隐藏规则配置器
- [x] 规则配置器可以添加规则
- [x] 规则配置器可以删除规则
- [x] 规则条件支持：用户角色、请假天数、金额等
- [x] 至少选择 1 个流程才能进入下一步
...
**验收标准：**
- [x] 显示完整配置预览
- [x] 预览内容包括：基本信息、模板、能力、流程、规则
- [x] 点击"保存草稿"，保存但不激活
- [x] 点击"激活并发布"，调用后台 API 激活
- [x] 激活成功，显示成功提示，跳转到详情页
- [x] 激活失败，显示错误信息，停留在当前页
...
**验收标准：**
- [x] 数据库迁移脚本执行成功
- [x] `system_users` 表增加 `user_type` 和 `agent_id` 字段
- [x] 索引创建成功
- [x] 创建 Agent 时自动创建用户记录
- [x] 用户记录的 `user_type` 为 `agent`
- [x] 用户记录的 `agent_id` 关联正确
- [x] 用户名格式为 `agent_{agentKey}`
- [x] 事务回滚正常（创建失败时用户记录也回滚）
...
**验收标准：**
- [x] API 接口实现正确
- [x] 可以查询部门的所有成员（真实员工 + 数字员工）
- [x] 可以按 `userType` 筛选
- [x] 返回数据包含 `userType` 和 `agentId` 字段
- [x] 数字员工有特殊标识（前端可区分）
- [x] 单元测试覆盖
...
**验收标准：**
- [x] 监听器可以正常注册到 Flowable
- [x] 可以监听到流程节点启动事件
- [x] 可以识别 AgentX 扩展节点类型
- [x] 可以获取节点配置参数
- [x] 可以设置执行结果到流程变量
- [x] 有完整的日志记录
- [x] 异常不会导致流程卡死
